	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		response.setContentType("text/html");
 		...
 		try
 		{
			...
			ResultSet resultSet=st.executeQuery("SELECT * FROM customer where mobile = "+mobile);
			while(resultSet.next())
			{
			pwd1=resultSet.getString(5);
	   ...
	   try
		{
			...
			Statement st=con.createStatement();
			ResultSet resultSet=st.executeQuery("SELECT * FROM customer where cid = "+n);
			while(resultSet.next())
			{
			data[0]=resultSet.getString(2);
			data[1]=resultSet.getString(3);
			data[2]=resultSet.getString(5);
